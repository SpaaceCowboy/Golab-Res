...
import { iraniandishes,subcategory } from "@/lib/data";
import {useTranslations} from 'next-intl';
import { FoodSlider } from "@/components/menu/food-slider";
import { Link } from "@/i18n/routing";

export default function Iraniandishes() {
  const t =  useTranslations("Menu");
  ["margarita", "mojito", "espressoMartini", "bikiniMartini"]
  const [searchTerm, setSearchTerm] = useState("");
  const categories = Array.from(new Set(iraniandishes.map(item => item.category)));
  ...
  />
    <div className="container-custom">       
      {!searchTerm && categories.map((category) => {
        const categoryItems = iraniandishes.filter(item => item.category === category);
  
        return (
          
          <div key={category} id={category.toLowerCase().replace(/\s+/g, '-')} className="mt-[20vh] md:mt-0 mb-16 scroll-mt-24">
            
            <h2 className="text-2xl font-medium mb-8 pb-2 border-b border-restaurant-primary/30  lg:mt-0">{category}</h2>
